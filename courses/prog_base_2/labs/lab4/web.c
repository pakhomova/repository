...
static url_params_t *url_params_parse (const char *urlParamsStr) {  //разбиваем строку юрл, котора€ передаетс€, на пары ключ-значение
...
static const char *url_params_get (url_params_t *url_params, const char *key) { //получаем значение параметра по данному ключу
    int i;
...
int callback (pensioners_t *pensioners, http_method_t http_method, const char *url, const char *request, char *response) { //коллбэк обрабатывает различные ситуации(в зависимости от юрл) и на основе этого выполн€ет нужные действи€
    if (strcmp(url, "/api/pensioners") == 0) {
        if (http_method == http_method_get) { //формируем ответ на гет-запрос
            cJSON *json = cJSON_CreateArray();
            int i;
            for (i = 0; i < pensioners->number; i++) {
                pensioner_t *pensioner = pensioner_copy(&pensioners->pensionerArray[i]);
    ...
    if (strcmp(url, "/pensioners") == 0) { //если юрл такого вида, то выводим список ссылок на пенсионеров
        if (http_method == http_method_get) {//формируем ответ на гет-запрос
            ...
            for (i = 0; i < pensioners->number; i++) {
                ...
                sprintf (hrefBuffer, "<a href=\"/pensioners/%u\">", pensioner->id);//делаем ссылку на пенсионера
            ...
            strcat (response, "</ul>");
            strcat (response, "<h2>Navigate to <a href=\"/new-pensioner\">new pensioner creation page</a></h2>");//ссылка на страницу-создание нового пенсионера
            strcat (response, "</body></html>");
